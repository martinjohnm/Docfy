...
export const useGetBookings = () =>{
    ...
    const [selectedPageNo, _setSelectedPageNo] = useRecoilState(selectedPageNumber)
    
    useEffect(() => {

        getBookingsFn({bookingFilter, selectedPageNo})

    }, [bookingFilter, selectedPageNo])

    


    const getBookingsFn = async ({bookingFilter, selectedPageNo} : {bookingFilter : BookingTypeToFetch, selectedPageNo : number})  => {
